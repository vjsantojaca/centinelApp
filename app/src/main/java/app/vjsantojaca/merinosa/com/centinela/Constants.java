package app.vjsantojaca.merinosa.com.centinela;

/**
 * Created by vsantoja on 14/08/15.
 */
public class Constants
{
    //SharedPreferences
    public static final String SENT_TOKEN_TO_SERVER = "sentTokenToServer";
    public static final String REGISTRATION_COMPLETE = "registrationComplete";

    //Info Server
    public static final String URL_SERVER = "http://192.168.1.96:8080/centinela/api";
    public static final String PATH_LOCATION = "/location";
    public static final String PATH_SYSTEM = "/system";
    public static final String PATH_GCM = "/device/gcm";
    public static final String PATH_CHECK = "/device/checkPass";
    public static final String PATH_SOUND = "/files/sound";
    public static final String PATH_PICTURE = "/files/picture";
}